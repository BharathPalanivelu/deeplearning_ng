...
model = load_vgg_model("pretrained-model/imagenet-vgg-verydeep-19.mat") #将图片输入到模型里，输出某一层的编码
print(model)
...
def compute_content_cost(a_C, a_G):
    ...
    a_C_unrolled = tf.reshape(tf.transpose(a_C[0],perm=[2,0,1]),[n_C,-1])
    a_G_unrolled = tf.reshape(tf.transpose(a_G[0],perm=[2,0,1]),[n_C,-1])
...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.reshape(tf.transpose(a_S[0],perm=[2,0,1]),[n_C,-1])
    a_G = tf.reshape(tf.transpose(a_G[0],perm=[2,0,1]),[n_C,-1])
...
generated_image = generate_noise_image(content_image) #随机初始化生成图像
...
model = load_vgg_model("pretrained-model/imagenet-vgg-verydeep-19.mat") #导入预训练过的模型
...
sess.run(model['input'].assign(content_image)) #内容图像成为VGG模型的输入

# Select the output tensor of layer conv4_2
out = model['conv4_2'] #选择层

# Set a_C to be the hidden layer activation from the layer we have selected
a_C = sess.run(out) #设a_C为模型第conv4_2层的激活

# Set a_G to be the hidden layer activation from same layer. Here, a_G references model['conv4_2'] 
# and isn't evaluated yet. Later in the code, we'll assign the image G as the model input, so that
# when we run the session, this will be the activations drawn from the appropriate layer, with G as input.
a_G = out  #a_G还没有被evaluated
